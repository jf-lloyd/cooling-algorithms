...
class IsingModel(Model):
    ...
    def __init__(self, device:"Device", params:dict):
        """
        params :
            'J'   : nearest-neighbour Ising coupling (float)
            'J2'  : diagonal next-nearest-neighbour coupling (float, default 0.)
        ...
        self.params = params
        self.hamiltonian_component_names = ["HJ", "HJ2", "Hg", "Hgx"]
    ...
    def name(self):
        ...
        J = self.params["J"]
        J2 = self.params.get("J2", 0.0)
        g = self.params["g"]
        gx = self.params["gx"]
        
        my_name = f"IsingModel{dim}D_Lx{Lx}Ly{Ly}J{J:.3f}J2{J2:.3f}g{g:.3f}gx{gx:.3f}"
        if dim == 1:
            if pbc:
                my_name+="_pbc"
        elif dim == 2:
    ...
    def next_nearest_neighbour_pairs(self):
        """Return square-lattice diagonal next-nearest-neighbour pairs."""
        ...
        for x in range(Lx):
            for y in range(Ly):
                for dx, dy in ((1, 1), (1, -1)):
                    ...
                    if pbc_y:
                        ty %= Ly
                    if not (0 <= tx < Lx and 0 <= ty < Ly):
                        continue
                    s = lattice.index(x, y)
                    t = lattice.index(tx, ty)
                    if s != t:
    ...
    def build_hamiltonian(self):
        ...
        # Diagonal next-nearest-neighbour XX term
        HJ2 = 0
        if J2 != 0.0:
            for (s, t) in self.next_nearest_neighbour_pairs():
                q1 = qubits[s]
                q2 = qubits[t]
                H_bond = J2 * (cirq.X(q1) * cirq.X(q2))
                HJ2 += H_bond
        ...
        H0 = HJ + HJ2 + Hg + Hgx
        return H0, [HJ, HJ2, Hg, Hgx], local_Hops

    def build_floquet_hamiltonians(self, order=1):
        h1, h2, h3 = self.hamiltonian_components[:3]
    ...
    def get_system_layer(self):
        ...
        if J2 != 0.0:
            XX2gate = cirq.XXPowGate(exponent=J2/(np.pi/2), global_shift=-0.5)
            for s, t in self.next_nearest_neighbour_pairs():
                q1 = qubits[s]
                q2 = qubits[t]
                system_gates.append(XX2gate(q1, q2))
