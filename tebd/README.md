...
- `ri_mps_cli.py` — runs the exact RI cooling circuit on an MPS (trajectory
  unravelling of the depolarizing channels and bath resets). Energy observable
  `H0 = +J<XX> - g<Z>`; OBC.
- `quimb_traj.py` — the underlying MPS-TEBD engine and measurement helpers
  (also a standalone MPI RI-cooling simulator).
